...
import 'package:tasks/data/attachments.dart';
import 'package:tasks/data/custom_user.dart';
import 'package:tasks/data/submissions.dart';
import 'package:tasks/services/attachments_db.dart';
import 'package:tasks/services/submissions_db.dart';
import 'package:tasks/services/updatealldata.dart';
import 'package:tasks/widgets/attachment_editor_composer.dart';
...
class _SubmissionComposerState extends State<SubmissionComposer> {
  ...
  Widget build(BuildContext context) {
    ...
    if(submissionList.isNotEmpty) {
      ...
      var userSubmission = submissionList.firstWhere((i) =>  i.user.uid == user.uid && i.classroom.className == widget.assignment.classroom.className);
      print("Reloaded");
      if(firstTime) {
      ...
      children: [
        ...
        Padding(
          ...
          child: MaterialButton(
            ...
            onPressed: () async {
print('Printing On Tap');
print(attachments);
              for(int i=0; i<attachments.length; i++) {
                print("Printing Attachments ${attachments.length}");
                String safeURL = attachments[i].url.replaceAll(new RegExp(r'[^\w\s]+'),'');
                print(safeURL);
                //
                await AttachmentsDB().createAttachmentStudentsDB(user.uid, attachments[i].url, safeURL, "${widget.assignment.classroom.className}__${widget.assignment.title}").then((value) {
                  print('attachmentsDB() function completed');
                });
              }
              ...
              await updateAllData();
   notificationsServices.sendNotification('Task Submitted', 'Thank you for completing the assignment');
